...
class EquiRectagularRenderer: NSObject, MTKViewDelegate {
    ...
    /*
        let image = NSImage(named: NSImage.Name("newport_loft.hdr"))
        let imageReps = image?.representations
        let imageRep = imageReps?[0] as! NSBitmapImageRep
        print(imageRep.bytesPerRow, imageRep.samplesPerPixel, imageRep.bitsPerSample, imageRep.bitsPerPixel)
        let cgImage = imageRep.cgImage
        print(cgImage!)
    */
        // The graphic is in the main bundle not in the Asset catalog.
        let bundle = Bundle.main
        // The HDR has 3 components R, G, B each a 32-bit Float  => 96 bits/pixel.
        // rgba16Float - four components R, G, B, A, each component is a 16-bit Float => 64 bits/pixel.
        // In MSL, rgba16Float is a half4 (4x2=8 bytes in size).
        let url = bundle.url(forResource: "newport_loft",
                             withExtension: "hdr")
        hdrTexture = loadEXRTexture(url!, device: self.device)
    ...
    func createCubemapTexture_RH() {
        ...
        // The +X, -X, +Z and -Z faces are generated correctly.
        // The other 2 faces are generated correctly but the order was wrong.
        // +Y should be top, bottom face is generated.
        // -Y should be bottom, top face is generated.
        // The order of these 2 faces must be swapped.
        let captureProjectionMatrix = matrix_perspective_right_hand(radians_from_degrees(90),
                                                                    1.0,
                                                                    0.1, 10.0)
        var captureViewMatrices = [float4x4]()
        var viewMatrix = matrix_look_at_right_hand(float3(0,0,0),   // eye is at the origin of the cube.
        ...
        // We can swap the Y+ and Y- faces here but
